...
import { LayoutDashboard, Zap, History, Clock, CheckCircle2, AlertCircle } from 'lucide-react';
  ...
  const handleGenerateEntrevista = async () => {
    ...
    try {
      const res = await fetch('https://sixmprueba.onrender.com/api/diagnostico', {
        ...
        body: JSON.stringify({ 
          prompt: `Eres un consultor experto en 6M. Analiza la falla: ${sintomas} en el equipo ${contexto}. Genera un JSON con este formato exacto: {"categorias": [{"nombre": "...", "preguntas": [{"texto": "..."}]}]}` 
        })
      });
  ...
  const handleGenerateACR = async () => {
    ...
    try {
      const res = await fetch('https://sixmprueba.onrender.com/api/diagnostico', {
        ...
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ 
          prompt: `Basado en estas respuestas: ${JSON.stringify(respuestas)}, genera un dictamen de causa raíz. Responde solo JSON: {"hipotesis": "..."}` 
        })
      });
      ...
      if (user) {
        await addDoc(collection(db, "customers", user.uid, "incidencias"), {
          ...
          solucion: data.hipotesis || "Analizado",
          estado: "pendiente",
        ...
        <div className="max-w-4xl mx-auto">
          ...
          {tabActiva === 'inicio' && (
            <div className="grid grid-cols-3 gap-6">
              ...
              <div className="bg-white p-8 rounded-3xl border shadow-sm">
                ...
                <p className="text-3xl font-black text-slate-900">{incidencias.length}</p>
            ...
            <div className="space-y-8">
              {!categorias.length && !reporte && (
                <div className="bg-white p-10 rounded-[2.5rem] shadow-xl border space-y-6">
                  ...
                  <textarea className="w-full p-4 bg-slate-50 rounded-xl border h-32" value={sintomas} onChange={(e)=>setSintomas(e.target.value)} placeholder="Describa la falla..." />
                  <button onClick={handleGenerateEntrevista} disabled={loading} className="w-full bg-slate-900 text-white p-5 rounded-xl font-black uppercase">{loading ? "Analizando..." : "Iniciar Protocolo"}</button>
                </div>
              )}
              ...
              {categorias.length > 0 && !reporte && (
                <div className="space-y-6">
                  ...
                  <button onClick={handleGenerateACR} disabled={loading} className="w-full bg-cyan-600 text-white p-6 rounded-3xl font-black uppercase shadow-lg">Obtener Dictamen</button>
                </div>
              )}

              {reporte && (
                <div className="bg-[#0F172A] text-white p-12 rounded-[3rem] shadow-2xl">
                  <h3 className="text-cyan-500 font-black text-xs uppercase mb-4 tracking-widest">Dictamen Final</h3>
                  <p className="text-3xl font-black italic border-l-4 border-cyan-500 pl-6">{reporte.hipotesis || "Análisis completado"}</p>
          ...
          {tabActiva === 'bitacora' && (
            <div className="space-y-4">
              <h2 className="text-2xl font-black text-slate-900 mb-6 uppercase italic">Historial de Fallas</h2>
              {incidencias.map((i) => (
