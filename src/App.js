...
function App() {
  ...
  // 2. LISTENER DE BITÁCORA (Base de conocimientos)
  useEffect(() => {
    if (user) {
  ...
  const enviarFeedback = async (id, fueExitosa, solucionIA) => {
    try {
      ...
      if (!fueExitosa) {
        const manual = prompt("Indica la solución real aplicada para registrarla como antecedente:");
        if (!manual) return;
      ...
      mostrarAviso("📋 Antecedente técnico actualizado en la bitácora.");
    } catch (e) { alert("Error al actualizar el registro."); }
  };

  // --- DIAGNÓSTICO CON ANTECEDENTES HISTÓRICOS ---
  const handleGenerateEntrevista = async () => {
    if (!sintomas || !contexto) return alert("Por favor, describe el contexto y los síntomas.");
    setLoading(true);
    setCategorias([]);
    setReporte(null);

    // Búsqueda de antecedentes similares
    ...
    if (coincidencia) {
      const fechaC = coincidencia.fecha?.toDate().toLocaleDateString() || "anterior";
      promptContextual = `[ANTECEDENTE]: El ${fechaC} se registró un caso similar solucionado con: "${coincidencia.solucion_final}". Considera esto como referencia, no como verdad absoluta. ${promptContextual}`;
    }
    ...
    try {
      ...
      if (data.categorias) {
        ...
        if (coincidencia) mostrarAviso(`ℹ️ Se encontró un antecedente similar (${coincidencia.fecha?.toDate().toLocaleDateString()}).`);
      }
  ...
  return (
    <div className="flex h-screen bg-[#F8FAFC] text-slate-900 overflow-hidden font-sans">
      ...
      <main className="flex-1 overflow-y-auto p-10 relative">
        {aviso && (
          <div className="fixed bottom-10 right-10 z-[100] bg-slate-900 text-white p-6 rounded-3xl shadow-2xl border-l-4 border-blue-500 flex gap-4 animate-in slide-in-from-right-10 max-w-sm">
                ...
                <div className="space-y-8 animate-in fade-in">
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                    <div className="bg-white p-8 rounded-[2.5rem] border border-slate-100 shadow-sm">
                      ...
                      <p className="text-5xl font-black mt-2 text-green-600">{incidencias.filter(i => i.estado === 'resuelto').length}</p>
                    </div>
                    <div className="bg-white p-8 rounded-[2.5rem] border border-slate-100 shadow-sm">
                      <p className="text-[10px] font-black text-slate-400 uppercase tracking-widest italic">Pendientes de Validación</p>
                      <p className="text-5xl font-black mt-2 text-yellow-500">{incidencias.filter(i => i.estado === 'pendiente').length}</p>
                    </div>
                  </div>
                </div>
              )}
                ...
                <div className="space-y-10 animate-in slide-in-from-bottom-5">
                  {!categorias.length && !reporte && (
                    <section className="bg-white p-12 rounded-[3.5rem] shadow-xl border border-slate-100 space-y-8">
                      <div className="text-center"><h2 className="text-4xl font-black italic uppercase mb-2">Nueva Auditoría</h2><p className="text-slate-400 font-medium italic">Protocolo de Diagnóstico ACR</p></div>
                      <div className="space-y-6">
                        <input className="w-full p-5 bg-slate-50 rounded-3xl border-none ring-1 ring-slate-200 outline-none focus:ring-2 focus:ring-blue-600 transition-all font-medium" value={contexto} onChange={(e)=>setContexto(e.target.value)} placeholder="Ej: Planta de Energía" />
                        ...
                        <button onClick={handleGenerateEntrevista} disabled={loading} className="w-full bg-slate-900 hover:bg-blue-600 text-white p-6 rounded-[2rem] font-black text-xl transition-all shadow-2xl flex items-center justify-center gap-3">
                          {loading ? "PROCESANDO..." : "INICIAR PROCESO 6M"} <ChevronRight />
                        </button>
                      </div>
                    </section>
                  )}
                  ...
                  {categorias.length > 0 && !reporte && (
                    <div className="space-y-12 pb-20">
                      <div className="flex items-center gap-4"><button onClick={() => setCategorias([])} className="p-3 bg-white rounded-2xl shadow-sm hover:bg-slate-50"><ArrowLeft size={20} /></button><h3 className="text-3xl font-black italic uppercase font-black uppercase tracking-tighter">Cuestionario Técnico</h3></div>
                        ...
                        <div key={idx} className="space-y-8">
                          ...
                          <div className="grid gap-6">
                            {cat.preguntas.map((pre, pIdx) => (
                              <div key={pIdx} className="bg-white p-8 rounded-[2.5rem] border border-slate-100 shadow-sm">
                      ...
                      <button onClick={handleGenerateACR} disabled={loading} className="w-full bg-green-600 text-white p-8 rounded-[3rem] font-black text-2xl shadow-2xl shadow-green-100 hover:bg-green-700 transition-all">
                        {loading ? "GENERANDO REPORTE..." : "GENERAR INFORME ACR"}
                      </button>
                    </div>
                  )}

                  {reporte && (
                    <section className="bg-slate-900 text-white p-12 rounded-[4rem] shadow-2xl space-y-12 border-t-[16px] border-blue-600">
                      ...
                      <div className="bg-blue-600/20 p-10 rounded-[3rem] border border-blue-500/30">
                        <h5 className="font-black text-2xl mb-4 flex items-center gap-3 text-blue-400 uppercase italic tracking-tighter"><Info /> Hipótesis Sugerida</h5>
                        <p className="text-blue-50 text-2xl font-medium italic leading-relaxed">"{reporte.hipotesis}"</p>
                      </div>
                      <button onClick={() => {setReporte(null); setCategorias([]); setContexto(''); setSintomas('');}} className="w-full py-6 text-slate-500 font-black uppercase hover:text-white transition-all italic tracking-widest text-sm text-center">Nueva Auditoría Técnica</button>
                    </section>
                  )}
                </div>
              )}
                ...
                <div className="space-y-8 animate-in fade-in pb-20">
                  ...
                  <div className="grid gap-8">
                    {incidencias.filter(i => i.titulo?.toLowerCase().includes(busqueda.toLowerCase()) || i.descripcion?.toLowerCase().includes(busqueda.toLowerCase())).map((item) => (
                      <div key={item.id} className={`bg-white p-10 rounded-[3.5rem] border transition-all ${item.estado === 'resuelto' ? 'border-green-100 shadow-sm' : 'border-slate-100 shadow-xl ring-1 ring-slate-200/50'}`}>
                        ...
                        {item.estado !== 'resuelto' ? (
                          <div className="bg-slate-50 p-8 rounded-[2.5rem] border border-dashed border-slate-300">
                            ...
                            <div className="flex gap-4">
                              <button onClick={() => enviarFeedback(item.id, true, item.solucion)} className="flex-1 bg-green-600 hover:bg-green-700 text-white py-4 rounded-2xl font-black text-sm transition-all flex items-center justify-center gap-2"><ThumbsUp size={16} /> CONFIRMAR CAUSA</button>
                              <button onClick={() => enviarFeedback(item.id, false, "")} className="flex-1 bg-white border border-slate-200 text-slate-600 py-4 rounded-2xl font-black text-sm hover:bg-slate-50 transition-all flex items-center justify-center gap-2"><ThumbsDown size={16} /> CORREGIR ANTECEDENTE</button>
                            </div>
                          </div>
                        ) : (
