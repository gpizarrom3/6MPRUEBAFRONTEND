...
2. Usa lenguaje de ingeniería moderado. JAMÁS: catastrófico, urgente, peligro.
...
RESPONDE SÓLO con este JSON:
{
  ...
  if (!user) return (
    <div className="min-h-screen bg-white flex items-center justify-center p-6 text-slate-900 text-center">
      <div className="max-w-md w-full space-y-8">
        ...
        <p className="font-bold text-slate-400 tracking-widest uppercase text-xs">Ingeniería Metalmecánica e Innovación</p>
  ...
  // --- VISTA SUSCRIPCIÓN (FONDO BLANCO) ---
  if (!isSubscribed) return (
    <div className="min-h-screen bg-slate-50 flex items-center justify-center p-6 text-slate-900 text-center">
      <div className="max-w-md bg-white border border-slate-200 p-12 rounded-[3rem] shadow-xl space-y-8">
        ...
        <p className="text-slate-500 font-medium">Habilite su acceso profesional para diagnósticos 6M.</p>
      ...
      <main className="flex-grow p-10 overflow-y-auto bg-white">
        ...
        {view === 'audit_start' && (
          <div className="max-w-2xl mx-auto py-20 space-y-10 animate-in zoom-in">
            <h2 className="text-5xl font-black text-center italic uppercase text-[#003B4C]">Nuevo Diagnóstico</h2>
            <div className="bg-slate-50 p-12 rounded-[3.5rem] border border-slate-200 shadow-sm space-y-8">
              <div className="space-y-2">
                <label className="text-[10px] font-black uppercase text-slate-400 ml-4">Ubicación de Planta</label>
                <input placeholder="Ej: Celda de Flotación 03" className="w-full p-6 bg-white rounded-3xl border border-slate-200 outline-none focus:border-[#003B4C] transition-all" onChange={e => setContexto(e.target.value)} />
              </div>
              <div className="space-y-2">
                <label className="text-[10px] font-black uppercase text-slate-400 ml-4">Síntomas Reportados</label>
                <textarea placeholder="Describa los hallazgos técnicos..." className="w-full p-6 bg-white rounded-3xl border border-slate-200 h-40 outline-none focus:border-[#003B4C] transition-all" onChange={e => setSintomas(e.target.value)} />
              </div>
              <button onClick={iniciarAuditoria} disabled={loading} className="w-full bg-[#A61D2E] text-white p-6 rounded-3xl font-black text-xl hover:bg-black transition-all shadow-lg uppercase italic">
                {loading ? <Loader2 className="animate-spin mx-auto" /> : "GENERAR PREGUNTAS 6M"}
              </button>
            </div>
          </div>
        )}
                  ...
                  {cat?.preguntas?.map((p, pidx) => {
                    ...
                    return (
                      <div key={pidx} className="bg-slate-50 rounded-[3rem] border border-slate-200 p-12 space-y-10 shadow-sm">
                        ...
                        <textarea className="w-full p-6 bg-white rounded-[2rem] border border-slate-200 text-slate-600 h-32 outline-none focus:border-[#003B4C]" placeholder="Notas técnicas adicionales..." onChange={(e) => setRespuestas({...respuestas, [`${idBase}-obs`]: e.target.value})} />
                      </div>
                    );
                  })}
                </div>
          ...
          <div className="max-w-5xl mx-auto mb-20 shadow-2xl rounded-[3rem] overflow-hidden border border-slate-200 bg-white">
            <div className="bg-[#003B4C] p-12 flex justify-between text-white">
              <div>
                ...
                <p className="text-slate-300 font-bold text-[10px] uppercase tracking-widest mt-1">Informe Técnico Innovattech</p>
              </div>
            ...
            <div className="p-16 space-y-12">
              <section className="space-y-4">
                <h3 className="text-[#A61D2E] font-black text-xs uppercase tracking-widest">I. Resumen de Hallazgos</h3>
              ...
              <section className="space-y-6">
                <h3 className="text-[#003B4C] font-black text-xs uppercase tracking-widest">II. Evaluación Ishikawa (6M)</h3>
              ...
              <section className="bg-slate-50 p-12 rounded-[3.5rem] border border-[#003B4C]/20 shadow-inner">
                <h3 className="text-[#A61D2E] font-black text-xs uppercase mb-4 opacity-80">III. Hipótesis de Ingeniería</h3>
