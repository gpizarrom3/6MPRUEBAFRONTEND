  ...
  return (
    <div className="min-h-screen bg-slate-50 font-sans text-slate-900">
        ...
        <section className="space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-700">
          <div className="text-center space-y-2">
            ...
            <p className="text-slate-500 font-medium text-lg">
              Describe lo que ocurre y nuestra IA te guiará en el diagnóstico.
            </p>
          </div>
          ...
          <div className="bg-white rounded-[32px] shadow-2xl shadow-blue-100 p-8 md:p-12 border border-slate-100 space-y-8">
            <div className="grid md:grid-cols-2 gap-8">
              ...
              <div className="space-y-3">
                ...
                <input 
                  ...
                  placeholder="Ej: Caldera N°2, Planta de envasado..." 
                />
              </div>
              ...
              <div className="space-y-3">
                ...
                <textarea 
                  ...
                  placeholder="Ej: Se escucha un golpeteo metálico y hay pérdida de presión..." 
                />
              </div>
            </div>
            ...
            <button 
              ...
              className="w-full group relative overflow-hidden bg-blue-600 text-white p-6 rounded-2xl font-black text-xl shadow-xl hover:bg-blue-700 transition-all active:scale-95 disabled:opacity-50"
            >
              <span className="flex items-center justify-center gap-3">
                {loading ? "ANALIZANDO TU PROBLEMA..." : "EMPEZAR DIAGNÓSTICO"} <ChevronRight />
              </span>
            </button>
          </div>
        </section>
        ...
        <section className="space-y-8">
          {questions.map((cat, catIdx) => (
            <div key={catIdx} className="bg-white rounded-3xl shadow-md border border-slate-100 overflow-hidden">
              <div className="bg-slate-900 text-white p-4 px-8 font-black uppercase tracking-widest text-sm flex justify-between items-center">
                <span>Categoría: {cat.nombre}</span>
              </div>
              <div className="divide-y divide-slate-100">
                {(cat.preguntas || []).map((p, qIdx) => (
                  <div key={qIdx} className="p-8 space-y-6">
                    <p className="text-xl font-bold text-slate-800 leading-tight">{p}</p>
                    <div className="flex flex-wrap items-center gap-6">
                      ...
                      <input 
                        className="flex-1 p-3 bg-transparent border-b-2 border-slate-100 focus:border-blue-400 outline-none text-base italic text-slate-500"
                        placeholder="Escribe algún detalle adicional aquí..."
        ...
        {questions.length > 0 && !reporte && (
          <div className="py-10">
            <button 
              ...
              disabled={loading}
              className="w-full bg-emerald-600 text-white p-8 rounded-[32px] font-black text-2xl shadow-2xl hover:bg-emerald-700 hover:-translate-y-1 transition-all flex items-center justify-center gap-4"
            >
              <ClipboardCheck size={36} /> {loading ? "GENERANDO INFORME..." : "GENERAR MI REPORTE FINAL"}
            </button>
          </div>
        )}
          ...
          <section id="reporte-final" className="bg-white rounded-[40px] shadow-2xl border-t-8 border-emerald-500 overflow-hidden animate-in zoom-in duration-500">
            ...
            <div className="p-10 space-y-10">
              ...
              <div className="bg-slate-50 p-6 rounded-2xl border border-slate-100">
                <h3 className="text-xs font-black text-blue-900 uppercase mb-2">Análisis Detallado (6M)</h3>
                <div className="grid md:grid-cols-2 gap-4">
                  {Object.entries(reporte.analisis_6m || {}).map(([key, val]) => (
                    <div key={key} className="bg-white p-4 rounded-xl border border-slate-200">
                      <span className="text-[10px] font-black text-blue-600 uppercase tracking-widest">{key}</span>
                      <p className="text-sm text-slate-600 mt-1 leading-tight">{val}</p>
                    </div>
                  ))}
                </div>
              </div>
              ...
              <div className="p-8 bg-blue-900 text-white rounded-[32px] shadow-2xl border-b-8 border-blue-950">
                <h3 className="text-2xl font-black mb-4 uppercase tracking-tighter flex items-center gap-3">
                   <Briefcase size={28} className="text-blue-400" /> Propuesta de Solución
                </h3>
      ...
      <footer className="text-center p-10 text-slate-400 text-xs font-bold border-t border-slate-200 mt-20">
        POWERED BY INTELIGENCIA ARTIFICIAL 6M • 2026
      </footer>
    </div>
  );
}
